...
const RecordItem = ({records}) => {
  ...
  return (
    ...
    <View className="mx-2 mb-2 rounded-md p-4 space-y-2 bg-gray-100">
      <Text className={`font-bold ${emergencyStatus[records.status]} py-1 px-3 rounded-lg self-start`}>
        {records.status.toUpperCase()}
      </Text>
 
      <View className="space-y-2 p-1">
        <View className="flex flex-row">
          <Text className="w-1/3 font-bold text-gray-500">Emergency type:</Text>
          <Text className="flex-1 font-bold">{records.type.toUpperCase()}</Text>
        </View>
